...
nyc.transit.df <- read_csv(file = "./data/NYC_Transit_Subway_Entrance_And_Exit_Data.csv",
                     ...
                     mutate(entry = ifelse(entry == "YES", TRUE, FALSE),            #Casting entry as logical
                          route1 = route1 %>% tolower() %>% toupper(),            #Fixing incorrect lowercase e for E, route1 obs 109
                          route5 = replace(route5, 109, NA))                      #Fixing incorrect 7 service, 6thAv-Rockefellr obs 1628
...
#User Function to collapse route1:11 but omit NA's, returns vector of strings
collapse.route <- function(x){
  paste0(na.omit(x), collapse = "/")
}

#Create Distinct Station tibbl (keeping original for entry/exit question)
distinct.route.df <- nyc.transit.df %>%
                     mutate(route = apply(select(., route1:route11), 1, collapse.route)) %>%  #Apply collapse, create new string var
                     select(line, station_name, route, everything(), -c(route1:route11))      #Tidy up order and remove old routes

#Count Distinct Stations by Line, Name, and Route
distinct.stations <- distinct.route.df %>% distinct(., line, station_name, route) %>% nrow()

#Count distinct stations ADA compiance
distinct.stations.df <- distinct.route.df %>%
                     distinct(., line, station_name, route, ada)

distinct.ada.comp <- distinct.route.df %>%
                     distinct(., line, station_name, route, ada) %>%
                     select(ada) %>% 
                     apply(., 2, sum, na.rm = TRUE)
distinct.ada.prop <- distinct.route.df %>%
                     distinct(., line, station_name, route, ada) %>%
                     select(ada) %>% 
...
atrain.ada.comp <- distinct.stations.df %>%
...
<img src="p8105_hw2_qn2119_files/figure-markdown_github/unnamed-chunk-3-1.png" width="90%" />
...
<img src="p8105_hw2_qn2119_files/figure-markdown_github/unnamed-chunk-3-2.png" width="90%" />
